...
PATH = r"C:\Users\test\Desktop\IR excel spreadsheets\Consumer Services.xlsx"


class ConsumerServices():
    def __init__(self):
        consumerServicesData = pandas.read_excel(r"C:\Users\test\Desktop\IR excel spreadsheets\Consumer Services.xlsx")
        self.consumerServicesDate = consumerServicesData.Date
        self.modelDate = self.consumerServicesDate.drop(self.consumerServicesDate.index[0])
        self.consumerServicesReturn = consumerServicesData.Level
        self.consumerARCHReturn = 100 * self.consumerServicesReturn.pct_change().dropna()
        self.xposition = ['3JANUARY2013', '3JANUARY2015', '3JANUARY2017']
        pandas.to_datetime(self.xposition)

    def timeseries(self):
        fig1 = plt.figure()
        consumerServicesGraph = fig1.add_subplot(111)
        consumerServicesGraph.plot(self.consumerServicesDate, self.consumerServicesReturn)

        consumerServicesGraph.set_title('Time Series')
        fig1.autofmt_xdate()
        fig1.draw
